...
class PlotConfigGenerator:
    ...
    def generate_plotting_config(
        ...
        if gradient_overlaps:
            attribute_config = {
                **attribute_config,
                **PlotConfigGenerator._get_student_grad_overlap_config(
                    student_hidden_layers
                    ),
                **PlotConfigGenerator._get_log_student_grad_overlap_config(
                    student_hidden_layers
                    )
                }
            ...
            for hidden_layer_index in range(len(student_hidden_layers)):
                attribute_config = {
                    ...
                    **PlotConfigGenerator._get_student_teacher_overlap_diff_config(
                            hidden_layer_index, num_teachers,
                            student_hidden_layers, teacher_hidden_layers
                        )
            }
    ...
    def _get_student_teacher_overlap_diff_config(
        ...
        for t in range(num_teachers):
            ...
            attribute_config[
                "Student-Teacher {} Overlap Differences".format(t)
                ] = \
                {
                "keys": student_teacher_t_overlaps,
                "plot_type": "scalar",
                "labels": [
                    r"$R^{0}_{{{1}{2}}} - R^{0}_{{{3}{4}}}$".format(
                        layer_index, i1, j1, i2, j2
                        )
                    for (i1, j1, i2, j2) in st_fill_combos
                ],
                "colours": PlotConfigGenerator.SHADES[t][:len(st_fill_combos)]
    ...
    @staticmethod
    def _get_student_grad_overlap_config(
        student_hidden_layers: List[int],
    ) -> Dict[str, Dict]:
    ...
    @staticmethod
    def _get_log_student_grad_overlap_config(
        student_hidden_layers: List[int],
    ) -> Dict[str, Dict]:
