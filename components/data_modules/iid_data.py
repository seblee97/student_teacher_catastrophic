...
class IIDDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_size: int, distribution: torch.distributions.Normal,
                 input_dimension) -> None:
        self._dataset_size = dataset_size
        self._data_distribution = distribution
        self._input_dimension = input_dimension

        self._fixed_dataset = self._data_distribution.sample((self._dataset_size,
...
class IIDData(_BaseData):
    ...
    def get_test_data(self) -> Constants.TEST_DATA_TYPES:
        ...
        """
        test_input_data = self._data_distribution.sample((self._test_batch_size,
                                                          self._input_dimension)).to(self._device)
    ...
    def get_batch(self) -> Dict[str, torch.Tensor]:
        ...
        if self._dataset_size == "inf":
            batch = self._data_distribution.sample((self._train_batch_size,
                                                    self._input_dimension)).to(self._device)
        else:
            try:
