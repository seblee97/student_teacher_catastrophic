...
class Argparser:
    ...
    def process_parser(parser):
        ...
        parser.add_argument(
            ...
            help='path to json for plot \
                config',
            default="plot_configs/summary_plots.json")
        parser.add_argument(
            '-auto_post_process',
            '--app',
            action='store_false',
        ...
        parser.add_argument(
            '-seed', '--s', type=int, help='seed to use for packages with prng', default=None)
        parser.add_argument(
            '-learner_configuration', '--lc', type=str, help="meta or continual", default=None)
        parser.add_argument(
            '-teacher_configuration',
            '--tc',
            type=str,
            help="noisy or independent or mnist",
            default=None)
        parser.add_argument(
            '-input_source', '--inp_s', type=str, help="mnist or iid_gaussian", default=None)
        parser.add_argument(
            '-input_dim', '--id', type=int, help="input dimension to networks", default=None)
        ...
        parser.add_argument(
            '-curriculum_type', '--ct', type=str, help="standard or custom", default=None)
        parser.add_argument(
            '-selection_type', '--st', type=str, help="random or cyclical", default=None)
        parser.add_argument(
            '-custom_curriculum', '--cc', type=str, help="order of teacher tasks", default=None)
        parser.add_argument(
            '-stopping_condition',
            '--sc',
            type=str,
            ...
            default=None)
        parser.add_argument(
            '-fixed_period', '--fp', type=int, help="time between teacher change", default=None)
        parser.add_argument(
            '-loss_threshold',
            '--lt',
            type=str,
        ...
        parser.add_argument(
            ...
            help='manually ensure zero self-overlap in initialisation \
                of student')
        parser.add_argument(
            '-zero_teacher_overlap',
            '--ztho',
            action='store_true',
            help='manually ensure zero self-overlap in initialisation \
                of teachers')
        parser.add_argument(
            '-learning_rate', '--lr', type=float, help="base learning rate", default=None)
        parser.add_argument(
            '-total_steps',
            '--ts',
            type=int,
            ...
            default=None)
        parser.add_argument(
            '-experiment_name', '--en', type=str, help="name to give to experiment", default=None)
        parser.add_argument(
            '-verbose', '--v', type=int, help="whether to display prints", default=None)
        parser.add_argument(
            '-tb_verbosity',
            '--tbv',
            type=int,
            ...
            default=None)
        parser.add_argument(
            '-checkpoint_path', '--cp', type=str, help="where to log results", default=None)
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            help="noise (in terms of std) \
                to be added to output of each teacher.",
            default=None)
        parser.add_argument(
            '-crop_start',
            type=float,
            help="For post-processing, where along \
                x domain to start the plot (as a percentage)",
            default=None)
        parser.add_argument(
            '-crop_end',
            type=float,
            help="For post-processing, where along \
                x domain to end the plot (as a percentage)",
            default=None)
        parser.add_argument(
            '-plot_figure_name',
            '--pfn',
            type=str,
            help="Save name for post-processing figure",
            default=None)
        parser.add_argument(
            '-individual_plot_figures',
            '--ipf',
            action='store_true',
        ...
        parser.add_argument(
            '-repeats', action='store_true', help="Flag states that save path is folder of repeats")
    ...
    def update_config_with_parser(args, params: Dict):
        ...
        if args.cc:
            custom = [int(op) for op in "".join(args.cc).strip('[]').split(',')]
        ...
        if args.lt:
            if isinstance(args.lt, float):
                ...
                threshold_sequence = [float(thr) for thr in "".join(args.lt).strip('[]').split(',')]
