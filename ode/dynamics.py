...
class StudentTeacherODE:
    ...
    def __init__(
        ...
        if curriculum is not None:
            self._curriculum = iter(curriculum[1:])
    ...
    def dr_dt(self) -> np.ndarray:
        ...
        for (i, n), _ in np.ndenumerate(derivative):
            ...
            for m, head_unit in enumerate(teacher_head):
                cov = self._configuration.generate_covariance_matrix([("i", i), ("n", n), (teacher_index, m)])
    ...
    def dq_dt(self) -> np.ndarray:
        ...
        for (i, k), _ in np.ndenumerate(derivative):
            ...
            sum_1 = 0
            for m, head_unit in enumerate(teacher_head):
                cov = self._configuration.generate_covariance_matrix([("i", i), ("k", k), (teacher_index, m)])
            ...
            sum_2 = 0
            for m, head_unit in enumerate(teacher_head):
                cov = self._configuration.generate_covariance_matrix([("k", k), ("i", i), (teacher_index, m)])
            ...
            for m, head_unit_m in enumerate(teacher_head):
                for n, head_unit_n in enumerate(teacher_head):
                    cov = self._configuration.generate_covariance_matrix([("i", i), ("k", k), (teacher_index, m), (teacher_index_2, n)])
                    sum_3 += head_unit_m * head_unit_n * self._i4_fn(cov)
            for m, head_unit_m in enumerate(teacher_head):
                for j, head_unit_j in enumerate(student_head):
                    cov = self._configuration.generate_covariance_matrix([("i", i), ("k", k), ("j", j), (teacher_index, m)])
    ...
    def du_dt(self) -> np.ndarray:
        ...
        for (i, p), _ in np.ndenumerate(derivative):
            ip_derivative = 0
            for m, head_unit in enumerate(teacher_head):
                cov = self._configuration.generate_covariance_matrix([("i", i), ("p", p), (teacher_index, m)])
