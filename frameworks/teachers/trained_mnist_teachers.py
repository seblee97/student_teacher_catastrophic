...
class TrainedMNISTTeachers(_BaseTeachers):
    ...
    def _setup_teachers(self, config: Dict):
        ...
        # hash contents of above config contents to see if trained teachers of this type exist
        ...
        if not os.path.exists(saved_weights_path):
            ...
            print("Teachers with given config do not have associated saved weights. Training classifiers...")

            os.makedirs(saved_weights_path)
            for t in range(self._num_teachers):
                mnist_classifier = train_mnist_classifier.MNISTTrainer(config=config, task_index=t)
                mnist_classifier.train()

                print("Teachers with given config trained. Saving weights...")

                mnist_classifier.save_model_weights(path=os.path.join(saved_weights_path, "teacher_{}.pt".format(t)))

            print("Teachers with config {} trained and saved.".format(config_hash))
        ...
        for t in range(self._num_teachers):
            ...
            teacher.load_weights(weights_path=os.path.join(saved_weights_path, "teacher_{}.pt".format(t)))
