...
class IIDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if idx == 0 and !(self.dataset_looped):
            self.dataset_looped = True    
        elif idx == 0 and !(self.repeat) and self.dataset_looped:
            raise IterationError("This IID dataset has already been looped through but repeat is off. Please check")
